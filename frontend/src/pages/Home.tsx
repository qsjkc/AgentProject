      ...
      <section className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        <div>
          ...
          <h1 className="mt-6 max-w-3xl font-serif text-5xl leading-tight text-slate-950 sm:text-6xl">
            把你的 AI Agent 项目落成真正可分发的桌宠桌面应用。
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">
            Detachym 负责把 Web 门户、账户系统、下载分发、桌宠客户端和后台管理串成一套完整产品。
            Web 门户负责预览、注册、下载和账户管理，桌面端负责透明桌宠、快捷聊天和主面板交互。
          </p>
      ...
      <section className="grid gap-6 md:grid-cols-3">
        <article className="rounded-[2rem] border border-slate-200 bg-white/85 p-6">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-600">
            支持置顶悬浮、托盘管理、主面板唤起与本地窗口状态保存，核心对话能力继续走云端 API。
          </p>
        </article>
        <article className="rounded-[2rem] border border-slate-200 bg-white/85 p-6">
          <div className="text-sm uppercase tracking-[0.25em] text-slate-500">用户门户</div>
          <h2 className="mt-4 text-2xl font-semibold text-slate-950">注册、偏好与下载安装</h2>
          <p className="mt-3 text-sm leading-7 text-slate-600">
            用户中心负责桌宠偏好、密码修改、版本查看和安装包下载，不再承担完整桌宠体验。
          </p>
        </article>
        <article className="rounded-[2rem] border border-slate-200 bg-white/85 p-6">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-600">
            管理员可以查看用户总览、启用或禁用账户、重置资料并维护知识库规模，适合首版单机云服务器场景。
          </p>
        </article>
      </section>
    </div>
