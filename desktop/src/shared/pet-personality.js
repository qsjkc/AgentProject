const personality = {
  cat: {
    createdReminder: (title, time, emailEnabled = false) => (
      `行吧，${time} 我会提醒你：${title}。${emailEnabled ? '到点也会发邮件。' : ''}`
    ),
  ...
  dog: {
    createdReminder: (title, time, emailEnabled = false) => (
      `收到！${time} 我一定提醒你：${title}！${emailEnabled ? '到点也会发邮件！' : ''}`
    ),
  ...
  pig: {
    createdReminder: (title, time, emailEnabled = false) => (
      `好哦，${time} 我会慢慢提醒你：${title}。${emailEnabled ? '到点也会发邮件给你。' : ''}`
    ),
